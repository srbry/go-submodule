...
pushd github.com/mailgun
  git submodule add git@github.com/mailgun/timetools.git
  cd timetools
...
pushd github.com/eapache
  git submodule add git@github.com/eapache/channels.git
  cd channels
  git checkout 1.1.0
popd`))
		Eventually(string(session.Out.Contents())).Should(ContainSubstring(`mkdir -p github.com/mesosphere
pushd github.com/mesosphere
  git submodule add git@https://github.com/containous/mesos-dns.git.git
popd`))
